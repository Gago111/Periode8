(function($, window, document, undefined) {
  ...
  $.extend(simpleCart.prototype, {
                    ...
                    <tr>\n\
                      ...
                      <tr>\
                        <td class='total'>Total</td>\n\
                         ...
                         <td class='total-geld text-right'>€ <i class='total-cart-cost'>0</i></td>\n\
                        </tr>\
                      </div>\n\
                    </table>\
                  </div>"
    ...
    _updateCartDetails: function() {
      ...
      $(this.options.totalCartCountClass).html(
        "Your Cart " + mi._totalCartCount() + " items"
      );
    ...
    _setEvents: function() {
      ...
      var mi = this;
      $(this.options.removefromCartClass).on("click", function(e) {
        e.preventDefault();
        var name = $(this).attr("data-name");
        var cost = Number($(this).attr("data-price"));
        mi._removeItemToCart(name, cost, 1);
        mi._updateCartDetails();
      });
    ...
    _removeItemToCart: function(name, price, count) {
    ...
    _displayCart: function() {
      ...
      //     output = "<h4>Your order <i class='fas fa-shopping-cart float-right'></i></h4" ;
      // }
      for (var i in cartArray) {
        var pricetest = cartArray[i].price;
        output +=
          "<table>\n\
                      <td class='options'><i class=' ml-3 sc-remove-from-cart' data-name='Enchiladas' data-price='1.25' type='submit'></i> " +
          cartArray[i].count +
          "\n\
                       " +
          cartArray[i].name +
          "</td>\n\
                       ...
                       <td class='text-right quantity-am font-weight-bold'>€ " +
          Math.round(pricetest * 100) / 100 +
          "</i></td>\
                       </tabel>";
      }
      return output;
    ...
    _totalCartCost: function() {
      ...
      for (var i in this.cart) {
        ...
        var fixTotalCost = Math.round(totalCost * 100) / 100;
      }
      return fixTotalCost;
    },
    _listCart: function() {
